...
function save() {
  ...
  try {
    ...
    $("#save-state").replaceChildren(node("span"), document.createTextNode("Saved on this device"));
  } catch {
...
function counts() {
  ...
  $("#export-edits").disabled = !changed.length || !!errors || !!state.stale;
...
function drawPreview(canvas, lines, page) {
  ...
  for (const line of lines.filter(line => line.box === page)) {
    const y = line.row * 13 + 2;
    for (const operation of line.operations) {
      ...
      } else if (operation.char !== undefined) {
        context.fillStyle = "#2e4630";
        data.font.glyphs[operation.char].rows.forEach((bits, row) => {
          for (let x = 0; x < 8; x++) if (bits & (0x80 >> x)) context.fillRect(operation.x + x, y + row, 1, 1);
        });
      } else {
    ...
    if (line.extent > data.rules.pixelLimit || line.cells > data.rules.glyphLimit) {
...
function updateFeedback(row, card) {
  ...
  status.append(node("strong", "", result.valid ? (row.editable ? "Fits the current text rules" : "Structured dialogue · read-only") : "Check this entry"));
  ...
  if (!result.lines.length) { content.append(node("p", "preview-side", "Fix the token or font error to restore the preview.")); return; }
  ...
  function paint() {
    ...
    for (const line of result.lines.filter(line => line.box === page)) {
      const over = line.extent > 144 || line.cells > 30;
      ...
      bar.append(fill); meter.append(node("span", "", `L${line.row + 1}`), bar,
        node("span", "", `${line.extent}/144px · ${line.cells}/30 glyphs`)); meters.append(meter);
    }
  }
...
function makeCard(row, first) {
  ...
  if (row.editable) {
    ...
    input.addEventListener("keydown", event => {
      if (event.key === "Enter") {
        event.preventDefault(); insert(event.ctrlKey || event.metaKey ? "<brk>" : "<br>");
      }
    });
    ...
    for (const [label, value] of [["＋ Line break", "<br>"], ["＋ Page break", "<brk>"]]) {
...
async function initialize() {
  ...
  const start = data.events.find(event => event.name.toLowerCase().includes("waking at the inn")) || data.events[0];
  state = {event: start.id, query: "", filter: "all", page: 0, edits: {}, japanese: {}, stale: null};
  ...
  $("#export-edits").addEventListener("click", () => {
    try { download("shiren-prose-changes.tsv", exportEdits(data, state.edits)); notify("Changes downloaded. The project importer will recheck them before applying."); }
  ...
  $("#edits-file").addEventListener("change", async event => {
    ...
    try {
      ...
      const merged = importEdits(await file.text(), data, state.edits);
      state.edits = merged; validateAll(); save(); state.event = "all"; state.filter = "edited"; state.page = 0;
      navigation(); render(); notify("Edits imported and checked. Your existing non-conflicting edits were kept.");
    } catch (error) { notify(error.message, true); }
    event.target.value = "";
  });
  $("#source-file").addEventListener("change", async event => {
    const file = event.target.files[0]; if (!file) return;
    try { if (file.size > 2_000_000) throw new Error("The file is larger than 2 MB."); await loadSource(await file.text()); notify("Japanese source loaded. The file stays in this browser."); }
    catch (error) { notify(error.message, true); }
    event.target.value = "";
  });
  ...
  $("#recover-draft").addEventListener("click", () => download("shiren-prose-recovery.json", state.stale, "application/json"));
