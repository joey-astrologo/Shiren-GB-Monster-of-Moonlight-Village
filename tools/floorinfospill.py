...
``--fusion-kit`` reproduces the original two-record history fixture from
``mesen_spawn_fusion_kit.lua`` at the same bank-6 builder boundary: Fusion Pot[2] plus
Manji Kabura+1 carrying all nine weapon seals. The live Lua kit now also offers an
all-nine-seal shield, but that independent screen-5 route is owned by
``fusioncountspill.py``; adding it here would exceed this deterministic save's free
inventory slots. This route can be combined with ``--fusion`` to cover a standing
Fusion Pot while the two history records are carried.
