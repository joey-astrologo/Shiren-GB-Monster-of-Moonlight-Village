...
def load_reloc_ok(path=None):
    """-> {(kind, key): site} from script/build-inputs/reloc_ok.tsv. See that file's header.
    ...
    """
    path = path or os.path.join('script', 'build-inputs', 'reloc_ok.tsv')
    out = {}
...
def load_box_aliases(path=None):
    """-> {box id: box id it renders instead}. See script/build-inputs/box_alias.tsv.
    ...
    screen.
    """
    path = path or os.path.join('script', 'build-inputs', 'box_alias.tsv')
    out = {}
    if not os.path.exists(path):
        return out
    for line in open(path, encoding='utf-8'):
...
def main():
    ...
    # DECLARED region -- justified in script/build-inputs/box_alias.tsv, not guessed by scanning for
    ...
    # (script/build-inputs/dte_ok.tsv, generated by `gbrun.py --dte-scan`). Guessing this per bank was
    ...
    geom_path = os.path.join('script', 'build-inputs', 'box_geometry.tsv')
    ...
    # table (or the message-queue rule) that script/build-inputs/reloc_ok.tsv attributes to a site
    ...
    # immediate_refs() in extract.py and `docs/TRAPS.md`.
    ...
    raw_applied = 0
    raw_spans = []
    rp = os.path.join('script', 'build-inputs', 'raw_patches.tsv')
    ...
    tp = os.path.join('script', 'build-inputs', 'tile_patches.tsv')
    ...
    # while translations and script/build-inputs/dte_ok.tsv are keyed on the original `loc`. Without
    ...
    if dte_stats:
        ...
        print("                        populate script/build-inputs/dte_ok.tsv with "
